...
The application maintainer should deploy the updated component with the app's normal Convex deployment before enabling lifecycle calls. `src/component/schema.ts` adds `lifecycleOperations`, `ownedResources`, and `lifecycleLocks` with indexes; existing tables need no migration or backfill. Do not import legacy configuration snapshots as ownership records. Regenerate the application's component API in its configured deployment. Scheduled actions and expiry mutations run in Convex; no external worker or cron installation is required.

Before enabling purchases, the Telnyx account owner should verify that the bound `TELNYX_API_KEY` can manage profiles, number orders, phone numbers, and the intended 10DLC resources. Keep that component bound to the same provider account for the lifetime of its records; credential rotation may replace a key for that account, not switch accounts underneath pending work or ownership tombstones. Review the supplied HTTPS webhook and the initial US-only destination allowlist in `src/lifecycleProvider.ts`. The app must authorize tenant scopes and brand/campaign IDs and install its binding callback before enqueueing with that callback.

Gate new submissions in the app during rollout or rollback. Lifecycle actions use the Convex scheduler, not the existing five-worker Workpool limit; budget for one poll every 30 seconds per pending operation and limit admission according to provider/account quotas. Assign an application operator to monitor operation status, failed scheduled functions, and undelivered callbacks using `lifecycle.list`, `lifecycle.get`, and the Convex dashboard. Keep the worker functions, callback handlers, tables, locks, and provider references deployed until outstanding operations are reconciled. Do not roll back to a pre-lifecycle component or delete its state while provider outcomes remain unresolved; disabling new submissions does not cancel an already submitted purchase.
